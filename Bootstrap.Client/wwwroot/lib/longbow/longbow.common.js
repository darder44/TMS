    ...
    $.extend({
        ...
        bc: function (options) {
            ...
            function success(result) {
                ...
                if (options.title) toastr[result ? 'success' : 'error'](options.title + (result ? "成功" : "失敗"));
            ...
            var ajaxSettings = {
                ...
                error: function (XMLHttpRequest, textStatus, errorThrown) {
                    ...
                    if (window.toastr) {
                        ...
                        toastr.error(XMLHttpRequest.status === 500 ? '後台應用程序錯誤' : errorThrown, '程序錯誤');
                    }
                    success(false);
                }
            };
        ...
        footer: function (options) {
            ...
            // 增加 1px 修覆 IE11 下由於小數點導致頁腳消失bug
    ...
    $.fn.extend({
        ...
        smartTable: function (options) {
            var settings = $.extend({
                ...
                showButtonText: true,
                showSearchButton: true,             //是否顯示搜索按鈕
                showColumns: true,                  //是否顯示所有的列
                showRefresh: true,                  //是否顯示刷新按鈕
                showToggle: true,                   //是否顯示詳細視圖和列表視圖的切換按鈕
                cardView: $(window).width() < 768,  //是否顯示詳細視圖
            ...
            if (settings.search) {
                $('.bootstrap-table .fixed-table-toolbar .search-input').tooltip({
                    sanitize: false,
                    title: '<div class="search-input-tooltip">輸入任意字符串全局搜索 </br> <kbd>Enter</kbd> 搜索 <kbd>ESC</kbd> 清除搜索</div>',
                    html: true
                });
        ...
        if ($.fn.bootstrapTable) {
            ...
            $.extend($.fn.bootstrapTable.defaults, {
                ...
                formatNoMatches: function formatNoMatches() {
                    return '<span>沒有找到匹配的記錄</span>';
                },
                formatPaginationSwitch: function formatPaginationSwitch() {
                    return '<span>隱藏/顯示分頁</span>';
                },
                formatPaginationSwitchDown: function formatPaginationSwitchDown() {
                    return '<span>顯示分頁</span>';
                },
                formatPaginationSwitchUp: function formatPaginationSwitchUp() {
                    return '<span>隱藏分頁</span>';
                },
                ...
                formatAutoRefresh: function formatAutoRefresh() {
                    return '<span>自動刷新</span>';
                },
                formatExport: function formatExport() {
                    return '<span>導出數據</span>';
                },
