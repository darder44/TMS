(function ($) {
    ...
    DataTable = function (options) {
        var that = this;
        this.options = $.extend(true, { delTitle: "刪除數據", saveTitle: "保存數據" }, DataTable.settings, options);
            ...
            $(cId).on('click', { handler: this.options.events[cId] }, function (e) {
                ...
                if (options.bootstrapTable !== null) {
                    ...
                    if (arrselections.length === 0) {
                        lgbSwal({ title: '請選擇要編輯的數據', type: "warning" });
                        return;
                    }
                ...
                'click .del': function (e, value, row, index) {
                    ...
                    if (idValue != undefined) {
                        ...
                        if ($.isArray(nodes) && nodes.length > 0) {
                            ...
                            text = "本刪除項含有級聯子項目</br>您確定要刪除 <span class='text-danger font-weight-bold'>" + row.Name + "</span> 以及子項目嗎？";
                        }
                    }
                    swal($.extend({}, swalDeleteOptions, { html: text })).then(function (result) {
                        if (result.value) {
                            ...
                            $.bc({
                                url: op.url, data: iDs, method: 'delete', title: '刪除數據', logData: data,
                                callback: function (result) {
